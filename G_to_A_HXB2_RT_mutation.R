...
ref_seq <- read.table("RT_reference.txt", header = TRUE, sep = "\t")
...
for (my_file in filenames){
      ...
      if(full_data$major_seq[i-2] == "G"){
        if(full_data$major_seq[i-1] == "A" || full_data$major_seq[i-1] == "G"){
          if(full_data$major_seq[i] == "A" || full_data$major_seq[i] == "G" || full_data$major_seq[i] == "T") {
            if(full_data$major_seq[i-2] == "G" && full_data$freq_A[i-2] >= 5 && full_data$nucl_ref[i-2] != "A"){
    ...
    if(full_data$depth_total[i] >= min_coverage_major){
      if(i%%3 == 0){
        if(full_data$STOP_sample[i-2] == "STOP"){
          ...
          if(full_data$freq_T[i-2] >= 5){
            if(full_data$freq_G[i-1] >= 5){
              if(full_data$freq_A[i] >= 5){
            ...
            } else if(full_data$freq_A[i-1] >= 5){
              if(full_data$freq_A[i] >= 5 || full_data$freq_G[i] >= 5){
                full_data$STOP_sample2[i-2] = "STOP minor"
                if(full_data$freq_A[i] >= 5){
  ...
  for (i in 1:nrow(full_data)){
    ...
    if(full_data$sample_Apobec[i] != "Apobec"){
      if(full_data$major_seq[i] == "G" && full_data$freq_A[i] >= 5 && full_data$nucl_ref[i] != "A") {
